#!/usr/bin/env python3
"""
check-active-identity.py — Fail CI on retired identity leaking onto an active surface.

SHR-032. Historical records may name older product identities, but current
runtime, docs, scripts, tests, and generated product surfaces must read as
Shiroe unless a narrow private evidence exception is documented below.

This script draws that line and enforces it. Anything not listed in ARCHIVED or
ALLOWLIST is active: a retired identity token there is a failure. Adding a path to
...
Scope note: this is an identity scan, not a rename tool. Current runtime files
must not carry retired compatibility spellings; private closure receipts remain
evidence and are never product authority.
...
# scan while still catching prefixed and filename forms.
LEGACY_RE = re.compile(r"\b" + "ze" + r"ref|\b" + "zr" + "f", re.IGNORECASE)
...
ALLOWLIST: dict[str, str] = {
    ...
    "memory/state/legacy-closure-receipts/": "Private generated F2 receipts quote legacy source filenames by design. They are operator evidence, not Product Shiroe source.",
    "memory/state/legacy-closure-backups/": "Private generated F2 backups preserve legacy source bytes verbatim until receipt review. Rewriting them would break evidence integrity.",

    # --- tests that exercise this scanner by injecting retired spellings ----
    ...
    ".github/workflows/shr-verify.yml": "Changed-path compatibility for old verification branches; not a product surface.",
}
